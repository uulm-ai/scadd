 ...
 */
private[defaultimpl] class DefaultContext[V, T] private[defaultimpl](val variableOrder: Comparator[Variable[V]]) extends Context[V, T] {

  val interner: Interner[DefaultDiagram[V, T]] = IdentityInterners.newWeakIdentityInterner()
  val restrictCache: Cache[(DefaultDiagram[V, T], Map[Variable[V], V]), DefaultDiagram[V, T]] = CacheBuilder.newBuilder.maximumSize(1000000).build()
  val applyBinaryOperationCache: Cache[(DefaultDiagram[V, T], (T, T) => T, DefaultDiagram[V, T]), DefaultDiagram[V, T]] = CacheBuilder.newBuilder.maximumSize(1000000).build()
  val applyUnaryOperationCache: Cache[(DefaultDiagram[V, T], (T) => T), DefaultDiagram[V, T]] = CacheBuilder.newBuilder.maximumSize(1000000).build()
